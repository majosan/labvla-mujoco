...
simulated tactile injection **and live 12×12 heatmap** (T006).

Extends mujoco_client_tactile.py:
- After generating the 124-dim tactile vector, remap to a 12×12 grid using
  GloveMapper and push it to a matplotlib TactileHeatmap window so the
  operator can watch pressure patterns evolve in real time.
- --no-viz disables the heatmap for headless/CI runs.

Protocol per connection (unchanged from T005):
  1. Server sends metadata frame (msgpack).
  2. Client sends observation frame (msgpack + ndarray encoding), including tactile.
...
def main():
    ...
    parser.add_argument("--viz_interval", type=int, default=1,
                        help="Update heatmap every N frames (1=every frame)")
    ...
    print(f"[mujoco_client] Heatmap: {'OFF (--no-viz)' if args.no_viz else f'ON (every {args.viz_interval} frame(s))'}")
    print()

    # ----- optional heatmap window + 124→(12,12) mapper -----
    mapper = GloveMapper()
    viz = None if args.no_viz else TactileHeatmap(
        title=f"Tactile Pressure — {args.object_type}"
    )
    ...
    try:
        for step in range(args.num_steps):
            ...
            # 3b. Remap 124-dim tactile → 12×12 grid; push to heatmap
            grid_12x12 = mapper.process_frame(tactile)
            if viz is not None and (step % args.viz_interval == 0):
            ...
            print(
                ...
                f"grid_max={grid_12x12.max():.3f} grid_sum={grid_12x12.sum():.2f}"
            )
