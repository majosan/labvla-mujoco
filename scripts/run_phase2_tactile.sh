#!/bin/bash
# Phase 2 + Tactile automated runner
# Starts LabVLA service in background, waits for ready, runs the TACTILE
# MuJoCo client (mujoco_client_tactile.py), cleans up.
...
LOG_FILE=$PROJECT_DIR/phase2_tactile_run.log
SERVICE_LOG=$PROJECT_DIR/phase2_tactile_service.log
OBJECT_TYPE=${OBJECT_TYPE:-beaker}   # override with: OBJECT_TYPE=spatula bash ...
...
echo "[Phase2-Tactile] Starting LabVLA inference service (background)..." | tee -a "$LOG_FILE"
...
PYTHONPATH="$PROJECT_DIR/LabVLA" \
  ...
  > "$SERVICE_LOG" 2>&1 &
SERVICE_PID=$!
echo "[Phase2-Tactile] Service PID: $SERVICE_PID" | tee -a "$LOG_FILE"
...
# Run TACTILE MuJoCo client (5 frames)
echo "" | tee -a "$LOG_FILE"
echo "[Phase2-Tactile] Running MuJoCo tactile client (5 frames, object=$OBJECT_TYPE)..." | tee -a "$LOG_FILE"
PYTHONPATH="$PROJECT_DIR/LabVLA" \
  "$PYTHON" scripts/mujoco_client_tactile.py \
...
echo "" | tee -a "$LOG_FILE"
echo "[Phase2-Tactile] Cleaning up service (PID $SERVICE_PID)..." | tee -a "$LOG_FILE"
kill $SERVICE_PID 2>/dev/null || true
wait $SERVICE_PID 2>/dev/null || true

if [ $CLIENT_EXIT -eq 0 ]; then
    echo "[Phase2-Tactile] ✅ Tactile closed-loop validation complete!" | tee -a "$LOG_FILE"
else
    echo "[Phase2-Tactile] ❌ Client exited with code $CLIENT_EXIT" | tee -a "$LOG_FILE"
    exit $CLIENT_EXIT
fi
